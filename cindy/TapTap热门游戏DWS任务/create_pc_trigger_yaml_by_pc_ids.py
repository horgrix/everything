"""查询 crawler.db 中 taptap_pc_hot_list_ids_hourly 表里 crawled_at
等于当前小时（本地时间）的数据，对 app_id 去重后按升序返回，并写回
config/tasks/user_trigger/TaptapPC热门游戏统计数据采集任务New.yaml
...
# 脚本位于 cindy/TaptapPC热门游戏下载榜采集任务/ 下，向上两级即项目根目录，
# 这样无论从哪个目录运行本脚本，都能定位到 crawler.db 和 YAML 配置。
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DB_PATH = PROJECT_ROOT / "crawler.db"
TABLE = "taptap_pc_hot_list_ids_hourly"
YAML_PATH = PROJECT_ROOT / "config/tasks/user_trigger/TaptapPC热门游戏统计数据采集任务New.yaml"
VAR_NAME = "app_id"
